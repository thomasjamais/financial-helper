...
function useBalances() {
  return useQuery({
    queryKey: ['balances'],
    queryFn: async () => (await axios.get('/v1/balances', { baseURL: import.meta.env.VITE_API_URL ?? 'http://localhost:8080' })).data,
    retry: false,
  })
}
...
function useSetConfig() {
  ...
  return useMutation({
    mutationFn: async (body: any) => (await axios.post('/v1/bitget/config', body, { baseURL: import.meta.env.VITE_API_URL ?? 'http://localhost:8080' })).data,
      ...
      <div className="mt-6 space-y-4">
        ...
        >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
            ...
            <input className="border p-2" name="secret" placeholder="API Secret" />
            <input className="border p-2" name="passphrase" placeholder="Passphrase" />
            <select className="border p-2" name="env" defaultValue="paper">
              <option value="paper">paper</option>
              <option value="live">live</option>
            </select>
          </div>
          <button className="mt-2 px-3 py-2 bg-blue-600 text-white rounded" type="submit" disabled={setConfig.isPending}>
            Save Config
          </button>
        </form>
        ...
        <div>
          ...
          {error ? (
            <p className="text-red-600 text-sm">{(error as any)?.response?.data?.error || 'Set config to view balances'}</p>
          ) : (
