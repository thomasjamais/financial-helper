...
export default function TradeIdeas() {
  ...
  const buyIdeas = (data || []).filter((s) => s.side === 'BUY')
  const sellIdeas = (data || []).filter((s) => s.side === 'SELL')

  const TradeIdeasTable = ({
    ideas,
    side,
  }: {
    ideas: typeof data
  ...
  return (
    <div className="space-y-6 bg-slate-900 rounded-lg p-6">
      ...
      {/* BUY Signals first */}
      <TradeIdeasTable ideas={buyIdeas} side="BUY" />

      {/* SELL Signals */}
      <TradeIdeasTable ideas={sellIdeas} side="SELL" />
    </div>
  )
}
