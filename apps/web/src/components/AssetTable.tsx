...
export default function AssetTable({ assets }: { assets: Asset[] }) {
  ...
  const [sortBy, setSortBy] = useState<'asset' | 'amount' | 'price' | 'value'>(
    'value',
  )
      ...
      <div className="overflow-x-auto">
        <table className="w-full">
          <thead className="bg-slate-700">
            <tr>
              <th
                className="text-left p-3 text-slate-300 cursor-pointer"
                onClick={() => setSortBy('asset')}
              >
                Asset
              </th>
              <th
                className="text-right p-3 text-slate-300 cursor-pointer"
          ...
          <tbody>
            {sorted.map((a) => (
              <AssetRow key={a.asset} a={a as any} />
            ))}
          </tbody>
        </table>
      </div>
