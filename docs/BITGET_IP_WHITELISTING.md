# Bitget API IP Whitelisting Guide

This guide explains how to find your local development IP and AWS API's public IP address, and configure them in Bitget for API access.

## Overview

- **Local Development**: When testing locally, Bitget sees your **local public IP address** (from your ISP)
- **AWS Production**: Your API runs on AWS ECS Fargate in **private subnets**. All outbound traffic (including Bitget API requests) goes through a **NAT Gateway** with a static **Elastic IP (EIP)**. This EIP is the IP address that Bitget will see for all API requests from AWS.
...
- ⚠️ **If your IP changes**, you'll need to update the whitelist in Bitget
...
This will output the IP address you need to whitelist in Bitget.
...
You can also make a test request from your API to see what IP Bitget sees:

```bash
# From your local machine, trigger a test request
curl -X GET http://$(terraform output -raw api_url)/v1/balances

# Or check CloudWatch logs after making a Bitget API call
aws logs tail /ecs/financial-helper-api --follow
```

## Configuring IP Whitelisting in Bitget

1. **Log in to Bitget**:
   - Go to [Bitget.com](https://www.bitget.com)
   - Navigate to **API Management** (usually under Account Settings → API)

2. **Edit your API key**:
   - Find the API key you're using for the bot
   - Click **Edit** or **IP Whitelist**

3. **Add IP whitelist**:
   - Enable **IP Whitelist** or **Restrict access to trusted IPs only**
   - Add **both** IP addresses:
     - Your **local public IP** (for local development/testing)
     - Your **AWS NAT Gateway IP** (for production)
   - You can add multiple IPs (one per line or comma-separated, depending on Bitget's interface)
...
4. **Important Notes**:
   ...
   - ⚠️ Your **local IP may change** - if it does, update the whitelist in Bitget

## Verifying the Configuration

After whitelisting, test your Bitget API connection:
...
curl -X GET http://localhost:8080/v1/balances

# Or if using a different port
curl -X GET http://localhost:3000/v1/balances
```
...
2. Wait a few more minutes for Bitget to propagate the change
3. Verify the IP is added correctly in Bitget API settings
...
2. **Whitelist all IPs** in Bitget (comma-separated or one per line)
...
2. Update the whitelist in Bitget
...
2. **Check if your local IP changed**:
   ...
   # Compare with what's whitelisted in Bitget
   # If different, update the whitelist
   ```

3. **Check Bitget API key settings**:
...
4. **Check CloudWatch logs**:
   ...
   aws logs tail /ecs/financial-helper-api --follow
   ```
   Look for Bitget API errors and correlation IDs.
...
### Testing with Bitget Demo/Paper Trading

For local development, consider using **Bitget Demo/Paper Trading** which may have less strict IP requirements:

- Demo API: Use paper trading environment
- Demo API keys: Create separate keys in demo environment

This way you can test locally without worrying about IP whitelisting affecting your production keys.

## Security Best Practices

1. **Always use IP whitelisting** for production Bitget API keys
2. **Use separate API keys** for:
   - Paper trading vs live trading
   - Local development vs production
   - Different environments (demo vs production)
3. **Rotate API keys** periodically
4. **Monitor API usage** in Bitget dashboard
